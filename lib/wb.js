...
/**
 * Получить sticker → record из API supplier/returns с кэшированием и retry при 429.
 * Идентично getWBSalesMap по паттерну, но использует /api/v1/supplier/returns.
 *
 * @param {string} wbToken - WB API token
 * @param {function} [log] - функция логирования
 * @param {function} [onWait] - callback(waitSec, attempt) при ожидании retry
...
async function getWBReturnsMap(wbToken, log = console.log, onWait = null) {
  ...
  if (wbReturnsCache.isFetching) {
    log('WB returns cache: concurrent fetch in progress, using stale cache')
  ...
  try {
    const dateFrom = wbReturnsCache.lastDate
      ? new Date(Math.max(
          new Date(wbReturnsCache.lastDate).getTime(),
          Date.now() - 4 * 24 * 60 * 60 * 1000
        )).toISOString().split('T')[0]
      : new Date(Date.now() - 120 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      if (res.status === 200 && Array.isArray(res.body)) {
        ...
        // Мерджим новые записи в существующий кэш (upsert по sticker)
        if (wbReturnsCache.data === null) {
          wbReturnsCache.data = res.body
        } else {
          const existing = new Set(wbReturnsCache.data.map(r => r.sticker))
          for (const record of res.body) {
            if (record.sticker && existing.has(record.sticker)) {
              const idx = wbReturnsCache.data.findIndex(r => r.sticker === record.sticker)
              if (idx !== -1) wbReturnsCache.data[idx] = record
            } else if (record.sticker) {
              wbReturnsCache.data.push(record)
              existing.add(record.sticker)
            }
          }
        }
      ...
      if (res.status === 429) {
        ...
        if (waitSec > 60) {
          log(`WB returns cache: 429 with long retry (${waitSec}s > 60s), skipping retry`)
          if (wbReturnsCache.bySticker) {
            log(`WB returns cache: using stale cache (${wbReturnsCache.bySticker.size} records)`)
            wbReturnsCache.fetchedAt = Date.now()
            wbReturnsCache.isFetching = false
            try { fs.writeFileSync(WB_RETURNS_CACHE_FILE, JSON.stringify({ data: wbReturnsCache.data, lastDate: wbReturnsCache.lastDate, fetchedAt: wbReturnsCache.fetchedAt })) } catch (e) { log(`WB returns cache: disk save error: ${e.message}`) }
            return wbReturnsCache.bySticker
          }
          log('WB returns cache: no stale cache, returning empty')
          return new Map()
        }
