/**
 * @file Кэш Wildberries — продажи, возвраты, заказы, стикеры
 * @module lib/wb
 *
 * Управляет 4 кэшами WB API (продажи, аналитика возвратов, заказы маркетплейса, стикеры заказов)
 * с сохранением на диск, TTL-обновлением (2ч), блокировкой конкурентных запросов (Map-based)
 * и обработкой 429 rate-limit с экспоненциальным backoff.
 *
 * @requires path
 * @requires fs
...
/**
 * Логгер по умолчанию для загрузки кэша с диска.
 * @param {string} msg
 */
const log = (...args) => auto(args.join(' '))
...
try {
  if (fs.existsSync(WB_SALES_CACHE_FILE)) {
    ...
    } else if (saved && saved.fetchedAt) {
      // Файл есть, но data = null (например, после пустого ответа WB или 429)
      // Инициализируем пустые Map, чтобы bySticker не был null и TTL-проверка работала
      wbSalesCache.data = null
      wbSalesCache.lastDate = saved.lastDate || null
      wbSalesCache.fetchedAt = saved.fetchedAt
      wbSalesCache.bySticker = new Map()
      wbSalesCache.byGNumber = new Map()
      log(`WB cache: loaded empty cache from disk (fetchedAt=${new Date(saved.fetchedAt).toISOString()})`)
    }
  }
} catch (e) {
...
// ── WB returns cache ──
const WB_RETURNS_CACHE_TTL = CACHE_TTL
...
const WB_ORDERS_CACHE_TTL = CACHE_TTL
...
// ── Кэш статусов сборочных заданий WB (POST /api/v3/orders/status) ──
const WB_ORDERS_STATUS_CACHE_TTL = 5 * 60 * 1000 // 5 мин
const WB_ORDERS_STATUS_CACHE_FILE = path.join(moduleRoot, 'cache', 'wb_orders_status_cache.json')

/**
 * @type {{ byOrderId: Map<number,{wbStatus:string,supplierStatus:string}>|null, fetchedAt: number, isFetching: boolean }}
 */
const wbOrdersStatusCache = {
  byOrderId: null,
  fetchedAt: 0,
  isFetching: false
}

// Загрузка кэша статусов с диска при старте
try {
  if (fs.existsSync(WB_ORDERS_STATUS_CACHE_FILE)) {
    const saved = JSON.parse(fs.readFileSync(WB_ORDERS_STATUS_CACHE_FILE, 'utf-8'))
    if (saved && Array.isArray(saved.orders)) {
      wbOrdersStatusCache.byOrderId = new Map()
      for (const item of saved.orders) {
        if (item.id) wbOrdersStatusCache.byOrderId.set(item.id, { wbStatus: item.wbStatus || '', supplierStatus: item.supplierStatus || '' })
      }
      wbOrdersStatusCache.fetchedAt = saved.fetchedAt || 0
      log(`[WB Orders Status] loaded from disk — ${wbOrdersStatusCache.byOrderId.size} orderIds`)
    }
  }
} catch (e) {
  log(`[WB Orders Status] disk load error (starting fresh): ${e.message}`)
}

// ── Кэш для WB statistics-api (supplier/orders) — sticker + srid ──
const WB_ORDERS_STICKERS_CACHE_TTL = CACHE_TTL
...
const wbOrdersStickersCache = {
  ...
  bySrid: null,      // Map<srid, {sticker, isCancel, cancelDate, isRealization, ...}>
  ...
  if (fs.existsSync(WB_ORDERS_STICKERS_CACHE_FILE)) {
    ...
    if (saved && Array.isArray(saved.data)) {
      ...
      for (const record of wbOrdersStickersCache.data) {
        if (record.srid) {
          wbOrdersStickersCache.bySrid.set(record.srid, {
      ...
      log(`[WB Orders Stickers] loaded from disk — ${wbOrdersStickersCache.data.length} records, ${wbOrdersStickersCache.bySrid.size} srid→status, lastDate=${wbOrdersStickersCache.lastDate || 'none'}`)
    } else if (saved && saved.fetchedAt) {
      // Файл есть, но data = null — инициализируем пустой Map
      wbOrdersStickersCache.data = null
      wbOrdersStickersCache.lastDate = saved.lastDate || null
      wbOrdersStickersCache.fetchedAt = saved.fetchedAt
      wbOrdersStickersCache.bySrid = new Map()
      log(`[WB Orders Stickers] loaded empty cache from disk (fetchedAt=${new Date(saved.fetchedAt).toISOString()})`)
    }
  }
} catch (e) {
...
async function getWBSalesMap(wbToken, log = console.log, onWait = null) {
  ...
  try {
    ...
    const dateFrom = wbSalesCache.lastDate
      ? new Date(Math.max(
          new Date(wbSalesCache.lastDate).getTime(),
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      if (res.status === 200 && Array.isArray(res.body)) {
        if (res.body.length === 0) {
          ...
          // Сохраняем обновлённый fetchedAt на диск (только если data не null)
      ...
      if (res.status === 429) {
        ...
        // Если retry > 60с — не ждём, отдаём stale-кэш, попробуем в следующий раз
        if (waitSec > 60) {
          log(`WB cache: 429 with long retry (${waitSec}s > 60s), skipping retry`)
          if (wbSalesCache.bySticker) {
            log(`WB cache: using stale cache (${wbSalesCache.bySticker.size} records)`)
            wbSalesCache.fetchedAt = Date.now()
            wbSalesCache.isFetching = false
            try { fs.writeFileSync(WB_SALES_CACHE_FILE, JSON.stringify({ data: wbSalesCache.data, lastDate: wbSalesCache.lastDate, fetchedAt: wbSalesCache.fetchedAt })) } catch (e) { log(`WB cache: disk save error: ${e.message}`) }
            return wbSalesCache.bySticker
          }
          log('WB cache: no stale cache, returning empty')
          return new Map()
        }
...
// ── WB Returns Cache (rebuild from analytics) ──
/**
 * Получить sticker → record из кэша аналитики возвратов.
 * API statistics-api/v1/supplier/returns более не доступен (404),
 * поэтому перестраиваем индекс по стикеру из данных analytics/goods-return.
 *
 * @param {string} wbToken - WB API token
 * @param {function} [log] - функция логирования
 * @param {function} [onWait] - callback(waitSec, attempt) при ожидании retry
 * @returns {Promise<Map<string, Object>>}
 */
async function getWBReturnsMap(wbToken, log = console.log, onWait = null) {
  const now = Date.now()
  const previousBySticker = wbReturnsCache.bySticker
  ...
  // 2. Загружаем данные из analytics/goods-return и строим индекс по стикеру
  try {
    wbReturnsCache.isFetching = true

    // Убеждаемся, что кэш аналитики загружен (вызовет API при необходимости)
    await getWBAnalyticsReturnsMap(wbToken, log, onWait)

    // Перестраиваем bySticker из analytics data (ключ: stickerId → barcode → shkId)
    wbReturnsCache.bySticker = new Map()
    wbReturnsCache.data = []
    if (wbAnalyticsReturnsCache.data) {
      for (const record of wbAnalyticsReturnsCache.data) {
        const sticker = record.stickerId || record.barcode || record.shkId
        if (sticker) {
          wbReturnsCache.bySticker.set(String(sticker), record)
          wbReturnsCache.data.push(record)
        }
      }
    }
    // Пустой ответ аналитики не должен стирать рабочий stale-кэш возвратов.
    if (wbReturnsCache.bySticker.size === 0 && previousBySticker && previousBySticker.size > 0) {
      wbReturnsCache.bySticker = previousBySticker
      wbReturnsCache.data = Array.from(previousBySticker.values())
      wbReturnsCache.fetchedAt = Date.now()
      wbReturnsCache.isFetching = false
      log(`WB returns cache: analytics empty, keeping stale cache (${previousBySticker.size} records)`)
      return previousBySticker
    }
    wbReturnsCache.lastDate = wbAnalyticsReturnsCache.lastDate
    wbReturnsCache.fetchedAt = Date.now()

    // Сохраняем на диск
    try {
      fs.writeFileSync(WB_RETURNS_CACHE_FILE, JSON.stringify({
        data: wbReturnsCache.data,
        lastDate: wbReturnsCache.lastDate,
        fetchedAt: wbReturnsCache.fetchedAt
      }))
    } catch (e) {
      log(`WB returns cache: disk save error: ${e.message}`)
    }

    log(`WB returns cache: rebuilt from analytics (${wbReturnsCache.bySticker.size} records via stickerId/barcode)`)
    wbReturnsCache.isFetching = false
    return wbReturnsCache.bySticker
  } catch (e) {
 ...
 * @param {function} [onWait] - callback(waitSec, attempt) при ожидании retry
 * @returns {Promise<Map<string, Object>>}
 * @throws {Error} Если после всех retry не удалось получить данные и нет просроченного кэша
 */
async function getWBAnalyticsReturnsMap(wbToken, log = console.log, onWait = null) {
  ...
  try {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            if (res.status === 429) {
              ...
              // Если backoff > 60с — пропускаем, попробуем в следующий раз
              if (backoffSec > 60) {
                log(`[WB Analytics] backoff (${backoffSec}s) exceeds 60s threshold, skipping retry`)
                lastError = new Error(`WB Analytics rate limited — backoff ${backoffSec}s exceeds threshold`)
                break
              }
    ...
    const dateFrom = new Date(Math.max(
      new Date(wbAnalyticsReturnsCache.lastDate).getTime(),
      Date.now() - 4 * 24 * 60 * 60 * 1000
    )).toISOString().split('T')[0]
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        // WB иногда отвечает 200 без поля orders (или пустым массивом).
        // Это валидный пустой результат, а не ошибка сети: не запускаем retry.
        if (res.status === 200) {
          log('[WB Orders] empty/malformed 200 response — treating as empty page')
          hasMore = false
          pageSuccess = true
          break
        }
        ...
        if (res.status === 429) {
          ...
          // Если retry > 60с — не ждём, отдаём stale-кэш
          if (retrySec > 60) {
            log(`[WB Orders] 429 with long retry (${retrySec}s > 60s), skipping retry`)
            if (wbOrdersCache.byRid) {
              log('[WB Orders] using stale cache')
            ...
            log('[WB Orders] no stale cache, throwing to retry later')
            throw new Error('WB Orders rate limited (429) with long retry')
          }
    ...
    // Сохраняем отменённые заказы — WB API v3/orders не возвращает их после отмены
    /** @type {Array<Object>} */
    const cancelledOrders = []
    if (wbOrdersCache.byId && wbOrdersCache.byId.size > 0) {
      for (const [, order] of wbOrdersCache.byId) {
      ...
      if (cancelledOrders.length > 0) {
        log(`[WB Orders] preserving ${cancelledOrders.length} cancelled orders across rebuild`)
      }
    }
    ...
    // Восстанавливаем отменённые заказы, которые API больше не возвращает
    if (cancelledOrders.length > 0) {
      let restored = 0
      for (const order of cancelledOrders) {
        const id = String(order.id)
        if (!wbOrdersCache.byId.has(id)) {
          wbOrdersCache.data.push(order)
          wbOrdersCache.byId.set(id, order)
          if (order.rid) wbOrdersCache.byRid.set(order.rid, order)
          restored++
        }
      }
      if (restored > 0) log(`[WB Orders] restored ${restored} cancelled orders not returned by API`)
    }
...
// ═══════════════════════════════════════════════════════════════════
// WB: POST /api/v3/orders/status — статусы сборочных заданий
// ═══════════════════════════════════════════════════════════════════

/**
 * Запрашивает и/или возвращает из кэша статусы сборочных заданий WB.
 *
 * POST /api/v3/orders/status — возвращает wbStatus (sold, canceled_by_client, ready_for_pickup, sorted)
 * и supplierStatus для каждого orderId.
 *
 * @param {string} wbToken - WB API token
 * @param {number[]} orderIds - массив orderId (сборочных заданий)
 * @param {function} [log] - функция логирования
 * @returns {Promise<Map<number, {wbStatus: string, supplierStatus: string}>>}
 */
async function getWBOrdersStatus(wbToken, orderIds, log = console.log) {
  if (!orderIds || orderIds.length === 0) return new Map()

  const now = Date.now()

  // 1. Кэш свежий (< 5 мин) и покрывает все запрошенные ID — возвращаем подмножество
  if (wbOrdersStatusCache.byOrderId && (now - wbOrdersStatusCache.fetchedAt) < WB_ORDERS_STATUS_CACHE_TTL) {
    const allFound = orderIds.every(id => wbOrdersStatusCache.byOrderId.has(id))
    if (allFound) {
      log(`[WB Orders Status] HIT (${orderIds.length} IDs, ${Math.round((now - wbOrdersStatusCache.fetchedAt) / 1000)}s old)`)
      const result = new Map()
      for (const id of orderIds) {
        result.set(id, wbOrdersStatusCache.byOrderId.get(id))
      }
      return result
    }
  }

  // 2. Concurrent fetch — ждём и возвращаем что есть
  if (wbOrdersStatusCache.isFetching) {
    log('[WB Orders Status] concurrent fetch in progress, waiting...')
    for (let i = 0; i < 20; i++) {
      await new Promise(r => setTimeout(r, 500))
      if (!wbOrdersStatusCache.isFetching) break
    }
    if (wbOrdersStatusCache.byOrderId) {
      const result = new Map()
      for (const id of orderIds) {
        if (wbOrdersStatusCache.byOrderId.has(id)) result.set(id, wbOrdersStatusCache.byOrderId.get(id))
      }
      return result
    }
  }

  // 3. Фетчим — все ID (TTL истёк, кэш мог устареть)
  wbOrdersStatusCache.isFetching = true

  try {
    // Всегда запрашиваем все ID — даже те что есть в кэше могли изменить статус
    const idsToFetch = orderIds

    if (idsToFetch.length === 0) {
      log(`[WB Orders Status] all ${orderIds.length} IDs already in cache, returning`)
      const result = new Map()
      for (const id of orderIds) {
        if (wbOrdersStatusCache.byOrderId.has(id)) result.set(id, wbOrdersStatusCache.byOrderId.get(id))
      }
    ...
    // Дробим на батчи по 1000
    const BATCH_SIZE = 1000
    const allResults = []

    for (let i = 0; i < idsToFetch.length; i += BATCH_SIZE) {
      const batch = idsToFetch.slice(i, i + BATCH_SIZE)
      const body = JSON.stringify({ orders: batch })

      log(`[WB Orders Status] fetching batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(idsToFetch.length / BATCH_SIZE)} (${batch.length} IDs)...`)
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        if (res.status === 200 && res.body && Array.isArray(res.body.orders)) {
          allResults.push(...res.body.orders)
          log(`[WB Orders Status] batch ${Math.floor(i / BATCH_SIZE) + 1}: got ${res.body.orders.length} statuses`)
          success = true
          break
        }

        // Пустой/неполный ответ статусов не должен запускать минутные retry:
        // отсутствие статусов не мешает использовать сами заказы.
        if (res.status === 200) {
          log('[WB Orders Status] empty or malformed response — keeping empty status result')
          break
        }

        if (res.status === 429) {
          // Обновление статусов не должно блокировать основной кэш на минуты.
          log('[WB Orders Status] rate limited — keeping cached statuses')
          break
          if (retrySec > 60) {
            log('[WB Orders Status] 429 with long retry, skipping batch')
            break
          }
          await new Promise(r => setTimeout(r, retrySec * 1000))
          continue
        ...
        if (attempt < 3) {
          log(`[WB Orders Status] attempt ${attempt}/3 failed: ${res.status}, retrying...`)
          await new Promise(r => setTimeout(r, 1000))
          continue
        }
        log(`[WB Orders Status] batch failed after 3 attempts: ${res.status} ${typeof res.body === 'string' ? res.body : JSON.stringify(res.body || {})}`)
      }
    }
    ...
    for (const item of allResults) {
      if (!item.id) continue
      const statusInfo = { wbStatus: item.wbStatus || '', supplierStatus: item.supplierStatus || '' }
      if (wbOrdersStatusCache.byOrderId.has(item.id)) {
        const prev = wbOrdersStatusCache.byOrderId.get(item.id)
        if (prev.wbStatus !== statusInfo.wbStatus || prev.supplierStatus !== statusInfo.supplierStatus) {
          wbOrdersStatusCache.byOrderId.set(item.id, statusInfo)
          updated++
        }
      } else {
        wbOrdersStatusCache.byOrderId.set(item.id, statusInfo)
    ...
    wbOrdersStatusCache.fetchedAt = Date.now()
    log(`[WB Orders Status] merge: ${added} added, ${updated} updated (total ${wbOrdersStatusCache.byOrderId.size} orderIds)`)

    // 5. Сохраняем на диск
    try {
      const serialized = []
      for (const [id, info] of wbOrdersStatusCache.byOrderId) {
        serialized.push({ id, wbStatus: info.wbStatus, supplierStatus: info.supplierStatus })
      }
      fs.writeFileSync(WB_ORDERS_STATUS_CACHE_FILE, JSON.stringify({
        orders: serialized,
        fetchedAt: wbOrdersStatusCache.fetchedAt
      }))
      log(`[WB Orders Status] saved to disk: ${serialized.length} orderIds`)
    } catch (e) {
      log(`[WB Orders Status] disk save error: ${e.message}`)
    }

    // 6. Возвращаем подмножество для запрошенных ID
    const result = new Map()
    for (const id of orderIds) {
      if (wbOrdersStatusCache.byOrderId.has(id)) result.set(id, wbOrdersStatusCache.byOrderId.get(id))
    }
    ...
    wbOrdersStatusCache.isFetching = false
    log(`[WB Orders Status] fetch error: ${e.message}`, 'error')
    // Возвращаем что есть в кэше
    if (wbOrdersStatusCache.byOrderId) {
      const result = new Map()
      for (const id of orderIds) {
        if (wbOrdersStatusCache.byOrderId.has(id)) result.set(id, wbOrdersStatusCache.byOrderId.get(id))
 ...
 * @param {function} [log] - функция логирования
 * @param {function} [onWait] - callback(waitSec, attempt) при ожидании retry
 * @returns {Promise<Map<string, {sticker: string, isCancel: boolean, cancelDate: string, isRealization: boolean}>>} Map<srid, statusInfo>
 * @throws {Error} Если после всех retry не удалось получить данные и нет просроченного кэша
 */
async function getWBOrdersStickersMap(wbToken, log = console.log, onWait = null) {
  ...
  try {
    const dateFrom = wbOrdersStickersCache.lastDate
      ? new Date(Math.max(
          new Date(wbOrdersStickersCache.lastDate).getTime(),
          Date.now() - 4 * 24 * 60 * 60 * 1000
        )).toISOString().split('T')[0]
      : new Date(Date.now() - 120 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      if (res.status === 200 && Array.isArray(res.body)) {
        if (res.body.length === 0) {
          ...
          // Сохраняем обновлённый fetchedAt (только если data не null)
        ...
        // Build bySrid: srid → {sticker, isCancel, cancelDate, isRealization}
        wbOrdersStickersCache.bySrid = new Map()
        for (const record of wbOrdersStickersCache.data) {
          if (record.srid) {
            wbOrdersStickersCache.bySrid.set(record.srid, {
        ...
        // Save to disk (только если data не null)
        if (wbOrdersStickersCache.data !== null) {
          try {
            fs.writeFileSync(WB_ORDERS_STICKERS_CACHE_FILE, JSON.stringify({
              data: wbOrdersStickersCache.data,
      ...
      if (res.status === 429) {
        ...
        // Если retry > 60с — не ждём, отдаём stale-кэш
        if (retrySec > 60) {
          log(`[WB Orders Stickers] 429 with long retry (${retrySec}s > 60s), skipping retry`)
          if (wbOrdersStickersCache.bySrid) {
            log('[WB Orders Stickers] using stale cache')
            wbOrdersStickersCache.fetchedAt = Date.now()
            wbOrdersStickersCache.isFetching = false
          ...
          log('[WB Orders Stickers] no stale cache, throwing to retry later')
          throw new Error('WB Orders Stickers rate limited (429) with long retry')
        }
...
function mergeAnalyticsAndSales(analyticsMap, salesMap) {
  ...
  for (const [sticker, salesRecord] of salesMap) {
    if (!merged.has(String(sticker))) {
      // Пытаемся найти реальный orderId через srid → wbOrdersCache.byRid
      // (стикер может отличаться от orderId, а без резолва findInCache Step 3
      // вернёт orderId = sticker, что не даст найти заказ в МС)
      const _ordersByRid = wbOrdersCache.byRid || new Map()
      const _matchedOrder = salesRecord.srid ? _ordersByRid.get(salesRecord.srid) : null
      merged.set(String(sticker), {
        orderId: _matchedOrder ? String(_matchedOrder.id) : sticker,
        stickerId: '',
...
// ═══════════════════════════════════════════════════════════════════
// Helpers for findInCache
// ═══════════════════════════════════════════════════════════════════

/**
 * Определяет статус заказа WB по stickers кэшу (join по rid = srid).
 * Использует wbOrdersStickersCache.bySrid для получения isCancel/isRealization.
 * @param {Object} order - Объект заказа WB (содержит rid)
 * @param {Map} stickersBySrid - Карта стикеров по srid (wbOrdersStickersCache.bySrid)
 * @returns {{ isCancel: boolean, cancelDate: string, isRealization: boolean }}
 */
function resolveStickerStatus(order, stickersBySrid) {
  if (!order || !order.rid || !stickersBySrid || !stickersBySrid.size) {
    return { isCancel: false, cancelDate: '', isRealization: false }
  }
  const stickerObj = stickersBySrid.get(order.rid)
  if (!stickerObj) {
    return { isCancel: false, cancelDate: '', isRealization: false }
  }
  // WB statistics API возвращает cancelDate = '0001-01-01T00:00:00' для неотменённых
  const rawDate = stickerObj.cancelDate || ''
  const cancelDate = (!rawDate || rawDate.startsWith('0001') || rawDate === '01.01.0001') ? '' : rawDate
  return {
    isCancel: !!stickerObj.isCancel,
    cancelDate: cancelDate,
    isRealization: !!stickerObj.isRealization
  }
}

/**
 * Определяет сумму к выплате (forPay) для заказа WB по rid, стикеру и штрихкоду.
 * Последовательность: rid → stickersBySrid → salesMap → barcode (skus).
 * @param {Object} order - Объект заказа WB (содержит rid и/или skus)
 * @param {Map} salesMap - Карта продаж по стикеру (wbSalesCache.bySticker)
 * @param {Map} stickersBySrid - Карта стикеров по srid (wbOrdersStickersCache.bySrid)
 * @param {Object} wbSalesCache - Кэш продаж (с полем data для поиска по barcode)
 * @returns {number} Сумма к выплате (0 если не найдена)
 */
function resolveForPay(order, salesMap, stickersBySrid, wbSalesCache) {
  let forPay = 0
  if (order.rid) {
    const stickerObj = stickersBySrid.get(order.rid)
    const sticker = stickerObj ? stickerObj.sticker : null
    if (sticker) {
      const salesRec = salesMap.get(String(sticker))
      if (salesRec && salesRec.forPay) forPay = salesRec.forPay
    }
    // Fallback: если стикер пустой или не найден — ищем по srid напрямую
    if (!forPay) {
  ...
  // Fallback: поиск по barcode (skus) заказа в данных продаж
  if (!forPay && order.skus && Array.isArray(order.skus) && wbSalesCache.data) {
    for (const sku of order.skus) {
      for (const sr of wbSalesCache.data) {
        if (sr.barcode && String(sr.barcode) === String(sku) && sr.forPay) {
          forPay = sr.forPay
          break
        }
      }
...
/**
 * Поиск записи в объединённой карте (mergedMap) по orderId.
 * Сначала точное совпадение ключа, затем поиск по полю orderId.
 * @param {Map} mergedMap - Объединённая карта аналитики+продаж
 * @param {string|number} orderId - ID заказа
 * @returns {Object|null} Найденная запись или null
 */
function findMergedRecord(mergedMap, orderId) {
  let record = mergedMap.get(String(orderId))
  if (!record) {
    for (const [, r] of mergedMap) {
      if (String(r.orderId) === String(orderId)) { record = r; break }
    }
  }
  return record || null
}
...
/**
 * Производный статус WB из булевых флагов stickers cache.
 * WB API supplier/orders (flag=0) не возвращает поле status,
 * только isCancel/isRealization. Преобразуем их в строку.
 * @param {boolean} isCancel
 * @param {boolean} isRealization
 * @returns {string} 'cancelled' | 'delivered' | ''
 */
function deriveWbStatus(isCancel, isRealization) {
  if (isCancel) return 'cancelled'
  if (isRealization) return 'delivered'
  return ''
}
...
/**
 * Поиск по всем кэшам WB (мгновенно, без запросов к API).
 *
 * Порядок поиска:
 * 1. wbOrdersCache.byId — по id
 * 2. wbOrdersCache.data — по skus
 * 3. mergedMap (analytics+sales) — по sticker/orderId
 * 4. wbSalesCache.bySticker — по sticker/barcode
 * 5. wbOrdersStickersCache.bySrid — по sticker value
 *
 * Возвращаемые поля статуса:
 * - isCancel: boolean — отменён на маркете (из stickers cache)
 * - cancelDate: string — дата отмены
 * - isRealization: boolean — реализован на маркете (из stickers cache)
 *
...
function findInCache(code) {
  ...
  if (ordersById.size > 0) {
    ...
    if (mpRecord) {
      // Cross-reference: rid → sticker → forPay + mergedMap enrichment
      let forPay = resolveForPay(mpRecord, salesMap, stickersBySrid, wbSalesCache)
      const mergedRecord = findMergedRecord(mergedMap, mpRecord.id)
      if (mergedRecord && !forPay && mergedRecord.forPay) forPay = mergedRecord.forPay
      const stickerStatus = resolveStickerStatus(mpRecord, stickersBySrid)
      return {
        ...
        status: mergedRecord?.status || mpRecord.orderStatus || mpRecord.status || deriveWbStatus(stickerStatus.isCancel, stickerStatus.isRealization),
        isCancel: stickerStatus.isCancel,
        cancelDate: stickerStatus.cancelDate,
        isRealization: stickerStatus.isRealization,
  ...
  if (wbOrdersCache.data) {
    for (const order of wbOrdersCache.data) {
      if (order.skus && Array.isArray(order.skus) && order.skus.some(sku => String(sku) === String(code))) {
        ...
        const forPay = resolveForPay(order, salesMap, stickersBySrid, wbSalesCache)
        const stickerStatus = resolveStickerStatus(order, stickersBySrid)
        return {
          ...
          returnType: '',
          reason: '',
          status: order.orderStatus || order.status || deriveWbStatus(stickerStatus.isCancel, stickerStatus.isRealization),
          isCancel: stickerStatus.isCancel,
          cancelDate: stickerStatus.cancelDate,
          isRealization: stickerStatus.isRealization,
  ...
  if (mergedMap && mergedMap.size > 0) {
    ...
    if (record) {
      // Если orderId не резолвлен (равен исходному коду) — не используем mergedMap,
      // пусть Step 4 (salesMap) попробует с резолвингом через srid
      if (record.orderId && String(record.orderId) === String(code)) {
        // fall through к Step 4
      } else {
        ...
        return {
          ...record,
          _source: 'mergedMap',
          status: record.status || deriveWbStatus(
            record.isCancel !== undefined ? record.isCancel : false,
            record.isRealization !== undefined ? record.isRealization : false
          ),
          isCancel: record.isCancel !== undefined ? record.isCancel : false,
          isRealization: record.isRealization !== undefined ? record.isRealization : false,
        }
      }
    }
  }
  ...
  if (salesMap && salesMap.size > 0) {
    ...
    if (salesRecord) {
      // Резолвим реальный WB orderId через srid → ordersByRid → order.id
      // (salesRecord.sticker — это ID стикера, а не номер заказа)
      let orderId = salesRecord.sticker || code
      if (salesRecord.srid) {
        const ordersByRid = wbOrdersCache.byRid || new Map()
        const marketOrder = ordersByRid.get(salesRecord.srid)
        if (marketOrder) {
      ...
      return {
        ...
        returnType: '',
        reason: '',
        status: salesRecord.status || 'sale',
        isCancel: !!(salesRecord.status && (salesRecord.status === 'cancel' || salesRecord.status === 'cancelled')),
        isRealization: !!(salesRecord.status && (salesRecord.status === 'sale' || salesRecord.status === 'delivered')),
        lastChangeDate: salesRecord.lastChangeDate || '',
        subjectName: '',
        _source: 'salesMap'
      }
  ...
  for (const [srid, stickerObj] of stickersBySrid) {
    ...
    if (String(stickerVal) === String(code) || String(srid) === String(code)) {
      // Cross-reference: sticker → forPay
      let forPay = 0
      const salesRec = salesMap.get(String(stickerVal))
      if (salesRec && salesRec.forPay) forPay = salesRec.forPay

      const marketOrder = ordersByRid.get(srid)
      return {
        orderId: marketOrder ? String(marketOrder.id) : String(stickerVal),
        ...
        barcode: '',
        shkId: '',
        stickerId: String(stickerVal),
        completedDt: marketOrder ? marketOrder.createdAt || '' : '',
        orderDt: '',
        returnType: '',
        reason: '',
        status: marketOrder
          ? (marketOrder.orderStatus || marketOrder.status || deriveWbStatus(stickerObj.isCancel, stickerObj.isRealization))
          : deriveWbStatus(stickerObj.isCancel, stickerObj.isRealization),
        isCancel: stickerObj.isCancel,
        cancelDate: stickerObj.cancelDate,
        isRealization: stickerObj.isRealization,
        lastChangeDate: '',
        subjectName: '',
  ...
  if (orderByRid) {
    const stickerObj = stickersBySrid.get(String(code))
    const stickerVal = stickerObj ? stickerObj.sticker : ''
    const forPay = resolveForPay(orderByRid, salesMap, stickersBySrid, wbSalesCache)
    return {
      ...
      shkId: '',
      stickerId: stickerVal,
      completedDt: orderByRid.createdAt || '',
      orderDt: '',
      returnType: '',
      reason: '',
      status: orderByRid.orderStatus || orderByRid.status || deriveWbStatus(
        stickerObj ? stickerObj.isCancel : false,
        stickerObj ? stickerObj.isRealization : false
      ),
      isCancel: stickerObj ? stickerObj.isCancel : false,
      cancelDate: stickerObj ? stickerObj.cancelDate : '',
      isRealization: stickerObj ? stickerObj.isRealization : false,
  ...
  if (wbOrdersCache.data && Array.isArray(wbOrdersCache.data)) {
    for (const order of wbOrdersCache.data) {
      if (String(order.id) === String(code)) {
        // Cross-reference: rid → sticker → forPay + mergedMap enrichment
        let forPay = resolveForPay(order, salesMap, stickersBySrid, wbSalesCache)
        const mergedRecord = findMergedRecord(mergedMap, order.id)
        if (mergedRecord && !forPay && mergedRecord.forPay) forPay = mergedRecord.forPay
        const stickerStatus = resolveStickerStatus(order, stickersBySrid)
        return {
          orderId: String(order.id),
          ...
          returnType: mergedRecord?.returnType || '',
          reason: mergedRecord?.reason || '',
          status: mergedRecord?.status || order.orderStatus || order.status || deriveWbStatus(stickerStatus.isCancel, stickerStatus.isRealization),
          isCancel: stickerStatus.isCancel,
          cancelDate: stickerStatus.cancelDate,
          isRealization: stickerStatus.isRealization,
...
/**
 * Обновление всех просроченных кэшей WB (продажи, аналитика, заказы, стикеры).
 * Обновляет только те кэши, у которых истёк TTL.
 *
 * @param {string} wbToken - WB API token
 * @param {function} [log] - функция логирования
...
async function refreshIfStale(wbToken, log = console.log) {
  ...
  if (!wbAnalyticsReturnsCache.byOrderId || wbAnalyticsReturnsCache.byOrderId.size === 0 || (now - wbAnalyticsReturnsCache.fetchedAt) >= WB_ANALYTICS_CACHE_TTL) {
    promises.push(
  ...
  if (!wbOrdersCache.byRid || wbOrdersCache.byRid.size === 0 || (now - wbOrdersCache.fetchedAt) >= WB_ORDERS_CACHE_TTL) {
    promises.push(
  ...
  if (!wbReturnsCache.bySticker || wbReturnsCache.bySticker.size === 0 || (now - wbReturnsCache.fetchedAt) >= WB_RETURNS_CACHE_TTL) {
    promises.push(
      getWBReturnsMap(wbToken, log).then(m => { results.returns = m }).catch(e => { log(`WB returns cache: refreshIfStale error: ${e.message}`); results.returns = wbReturnsCache.bySticker || new Map() })
    )
  }
  ...
  // Обновляем кэш статусов (POST /api/v3/orders/status) — после обновления заказов
  // Сбрасываем TTL чтобы гарантированно перезапросить все статусы (могли измениться)
  // Статусы запрашиваем только после обновления кэша заказов.
  // Иначе refresh продаж/аналитики с тестовым или старым byId запускает
  // лишний POST и может зависнуть на rate-limit.
  if (results.orders && wbOrdersCache.byId && wbOrdersCache.byId.size > 0) {
    wbOrdersStatusCache.fetchedAt = 0
    const orderIds = [...wbOrdersCache.byId.keys()].map(Number)
    await getWBOrdersStatus(wbToken, orderIds, log).catch(e => {
      log(`WB cache: refreshIfStale status error: ${e.message}`)
    })
  }
...
/**
 * Принудительное обновление ВСЕХ кэшей WB независимо от TTL.
 *
 * @param {string} wbToken - WB API token
 * @param {function} [log] - функция логирования
 * @returns {Promise<{sales: Map|null, analytics: Map|null, orders: Object|null, stickers: Map|null}>}
...
/**
 * Возвращает объединённую карту аналитики+продаж из текущих кэшей (без запросов к API).
 *
...
module.exports = {
  ...
  wbSalesCache,
  wbReturnsCache,
  wbAnalyticsReturnsCache,
  wbOrdersCache,
  wbOrdersStatusCache,
  wbOrdersStickersCache
}
