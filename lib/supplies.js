...
async function scanNewOrders(msToken, wbToken, ozonClientId, ozonApiKey, log, onProgress, filterStoreId = '_all', filterMarketplaces = 'wb,ozon', filterDateFrom = '', filterDateTo = '') {
  ...
  if (Date.now() - lastSuppliesRefresh > SUPPLIES_CACHE_COOLDOWN) {
    ...
    // Ожидаем загрузку кэшей — findInCache() ниже должен найти заказы
    const results = await Promise.allSettled([
      wbToken ? wb.refreshIfStale(wbToken, log) : Promise.resolve(),
      ozonClientId && ozonApiKey ? ozon.refreshIfStale(ozonClientId, ozonApiKey, log) : Promise.resolve()
    ])
    // Если все кэши упали — разрешить повтор при следующем скане
    const anyFailed = results.some(r => r.status === 'rejected')
    if (anyFailed) {
      const names = ['WB', 'Ozon']
      results.forEach((r, i) => {
        if (r.status === 'rejected') log(`Supplies: ${names[i]} cache error: ${r.reason?.message || r.reason}`)
      })
      lastSuppliesRefresh = 0
    }
  }
      ...
      if (marketplace === 'wb') {
        ...
        if (wbData) {
          ...
          if (wbData.isCancel !== undefined) {
            ...
            marketplaceIsDelivered = !!wbData.isRealization && !wbData.isCancel
            marketplaceStatus = wbData.isCancel ? 'cancelled'
              : wbData.isRealization ? 'delivered'
              : ''
          } else {
