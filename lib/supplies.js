  ...
  for (let i = 0; i < allOrders.length; i++) {
    ...
    try {
      ...
      if (marketplace === 'wb') {
        ...
        marketplaceFound = !!(salesRec || returnsRec || stickerInfo || analyticsRec)

        if (marketplaceFound) {
          if (shipmentNum === '5201087379' || shipmentNum === '5201087380') {
            log(`[SUPPLIES DEBUG] Заказ ${order.name} (shipment=${shipmentNum}):`)
            log(`[SUPPLIES DEBUG]   salesRec: ${!!salesRec}, returnsRec: ${!!returnsRec}, stickerInfo: ${!!stickerInfo}, analyticsRec: ${!!analyticsRec}`)
          }
          ...
          if (shipmentNum === '5201087379' || shipmentNum === '5201087380') {
            log(`[SUPPLIES DEBUG]   marketplaceFound=${marketplaceFound}, marketplaceIsCancelled=${marketplaceIsCancelled}, marketplaceIsDelivered=${marketplaceIsDelivered}, marketplaceIsReturn=${marketplaceIsReturn}`)
          }
        }
