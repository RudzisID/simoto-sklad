      ...
      if (marketplace === 'wb') {
        ...
        if (marketplaceFound) {
          if (shipmentNum === '5201087379' || shipmentNum === '5201087380') {
            ...
            log(`[SUPPLIES DEBUG]   salesRec: ${!!salesRec}, returnsRec: ${!!returnsRec}, stickerInfo: ${!!stickerInfo}, analyticsRec: ${!!analyticsRec}, orderRec: ${!!orderRec}, mpStatus: ${mpStatus}`)
          }

          // Определяем статус по приоритету (первое совпадение)
          // Приоритет: return → cancelled (from orders API) → cancelled (from sticker) → delivered (from orders API) → delivered (from sales) → delivered (from sticker) → processing
          if (returnsRec) {
