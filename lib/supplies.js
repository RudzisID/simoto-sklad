...
async function scanNewOrders(msToken, wbToken, ozonClientId, ozonApiKey, log, onProgress, filterStoreId = '_all', filterMarketplaces = 'wb,ozon', filterDateFrom = '', filterDateTo = '') {
    ...
    try {
      ...
      if (marketplace === 'wb') {
        ...
        if (marketplaceFound) {
          ...
          //   → delivered(sales) → delivered(sticker) → delivered(supplyId+scanPrice) →
          ...
          if (returnsRec) {
            ...
            marketplaceIsDelivered = true
            marketplaceStatus = 'delivered'
          } else if (orderRec?.supplyId && orderRec?.scanPrice !== null) {
            marketplaceIsDelivered = true
            marketplaceStatus = 'delivered'
  ...
  // Фоновое обновление кэша заказов WB (чтобы на следующем скане scanPrice был актуален)
