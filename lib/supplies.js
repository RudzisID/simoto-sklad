...
async function scanNewOrders(msToken, wbToken, ozonClientId, ozonApiKey, log, onProgress, filterStoreId = '_all', filterMarketplaces = 'wb,ozon', filterDateFrom = '', filterDateTo = '') {
    ...
    try {
      ...
      if (marketplace === 'wb') {
        ...
        if (marketplaceFound) {
          ...
          // Приоритет: return(stickers) → return(analytics) → cancel(orders) → cancel(sticker) →
          //   → wbStatus=sold → wbStatus=canceled →
          //   → wbStatus=ready_for_pickup/sorted → complete(orders) →
          //   → delivered(sales) →
          //   → pickup(supplyId) → wbStatus(другие) → mpStatus(другие) → processing
          if (returnsRec) {
            marketplaceIsReturn = true
            marketplaceStatus = 'return'
          } else if (analyticsRec && !['', 'none', 'sold', 'sale', 'delivered'].includes(analyticsRec.status || '')) {
            // analytics/goods-return — запасной источник для возвратов
            marketplaceIsReturn = true
            marketplaceStatus = 'return'
            ...
            marketplaceIsCancelled = true
            marketplaceStatus = 'cancelled'
          } else if (wbStatus === 'ready_for_pickup' || wbStatus === 'sorted') {
            marketplaceIsDelivered = true
            marketplaceStatus = 'delivered'
            ...
            marketplaceIsDelivered = true
            marketplaceStatus = 'delivered'
          } else if (orderRec?.supplyId) {
            marketplaceStatus = 'pickup'
        ...
        if (ozonData) {
          ...
          if (marketplaceIsReturn) {
            ...
            marketplaceStatus = 'delivered'
          } else if (ozonPosting?.status === 'awaiting_deliver') {
            marketplaceStatus = 'pickup'
          } else {
      ...
      if (!hasDemand) {
        if (!marketplaceFound) {
          ...
          canDemand = true
        } else if (marketplaceStatus === 'pickup') {
          recommendationType = 'waiting_pickup'
          recommendation = '📦 Собран, ожидает отправки'
          canCancel = false
        } else {
          recommendationType = 'waiting'
  ...
  // Фоновое обновление кэша заказов WB (чтобы recheckOrder видел актуальные supplyId/scanPrice)
  if (wbToken && wbCount > 0 && wb.getWBOrdersMap) {
    wb.getWBOrdersMap(wbToken, log).catch(e => {
      log(`Supplies: background orders refresh (non-fatal): ${e.message}`)
    })
  }
...
function recheckOrder(order) {
  ...
  if (marketplace === 'wb') {
    ...
    // wbStatus из кэша POST /api/v3/orders/status (обновляется перед recheckOrder в sse.js)
