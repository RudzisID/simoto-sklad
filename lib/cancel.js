...
async function cancelOrder(orderIdOrFull, orderFull, demandId) {
  ...
  try {
    ...
    // 3. Смена статуса + ОБЯЗАТЕЛЬНЫЙ salesChannel + attributes
    // КЛЮЧЕВОЕ: MoySklad требует salesChannel и attributes, если они есть/обязательны в заказе!
    const updatePayload = {
      state: {
        meta: {
      ...
      // Добавляем атрибут канала продаж (ORDER_CHANNEL)
      attributes: [
        {
          meta: {
