...
async function findProductByCode(code) {
  ...
  try {
    ...
    console.log(`Searching product by code: ${normalizedCode}`)

    const filterResult = await API.GET('entity/assortment', { filter: `code=${normalizedCode}`, limit: 10 })
    console.log(`Filter results count: ${filterResult.meta?.size || 0}`)

    if (filterResult.rows && filterResult.rows.length > 0) {
      const product = filterResult.rows[0]
      console.log(`Found product: ${product.name}, code: ${product.code}, type: ${product.meta?.type}, id: ${product.id}`)
    ...
    console.log(`Filter returned no results, trying search on entity/product: ${normalizedCode}`)
    const searchResult = await API.GET('entity/product', { search: normalizedCode, limit: 10 })
    console.log(`Search results count: ${searchResult.meta?.size || 0}`)
    ...
    if (searchResult.rows && searchResult.rows.length > 0) {
      ...
      console.log(`Found product via search: ${product.name}, code: ${product.code}, id: ${product.id}`)
      productCache.set(normalizedCode, product)
      return product
    }
