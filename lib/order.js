...
async function getOrderFull(orderId) {
  try {
    const API = getApi()
    return await API.GET('entity/customerorder/' + orderId + '?expand=demands,positions.assortment,state,returns,payments,store')
  } catch (e) {
...
async function getDemand(demandId) {
  try {
    const API = getApi()
    return await API.GET('entity/demand/' + demandId + '?expand=positions,salesChannel,returns,state')
  } catch (e) {
