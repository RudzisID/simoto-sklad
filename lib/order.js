...
// Кеш возвратов по ID отгрузки: Map<demandId, Array<{id, name, sum, demand}>>
// Сохраняется в файл для персистентности между перезапусками
const returnsByDemandCache = new Map()
let returnsCacheFullyLoaded = false

const CACHE_DIR = path.join(__dirname, '..', 'cache')
const CACHE_FILE = path.join(CACHE_DIR, 'returns_cache.json')

function loadReturnsCacheFromFile() {
  try {
    if (!fs.existsSync(CACHE_FILE)) return false
    const raw = fs.readFileSync(CACHE_FILE, 'utf-8')
    const data = JSON.parse(raw)
    if (!data || !data.returns || !Array.isArray(data.returns)) return false
    returnsByDemandCache.clear()
    for (const entry of data.returns) {
      if (entry.demandId && Array.isArray(entry.rows)) {
        returnsByDemandCache.set(entry.demandId, entry.rows)
      }
    }
    console.log(`[order.js] Loaded returns cache from file: ${returnsByDemandCache.size} demands`)
    return returnsByDemandCache.size > 0
  } catch (e) {
    console.warn('[order.js] Failed to load returns cache:', e.message)
    return false
  }
}

function saveReturnsCacheToFile() {
  try {
    ...
    const data = {
      ...
      returns: Array.from(returnsByDemandCache.entries()).map(([demandId, rows]) => ({
        demandId,
        rows
      }))
    }
    fs.writeFileSync(CACHE_FILE, JSON.stringify(data), 'utf-8')
    console.log(`[order.js] Returns cache saved to file: ${returnsByDemandCache.size} demands`)
  } catch (e) {
    console.warn('[order.js] Failed to save returns cache:', e.message)
  }
}
...
async function findSalesReturnsByDemand(demandId) {
  try {
    ...
    // Если кеш уже построен в памяти — возвращаем из кеша (мгновенно)
    if (returnsCacheFullyLoaded) {
      return { rows: returnsByDemandCache.get(demandId) || [] }
    }

    // Пробуем загрузить из файла (чтобы не делать 40+ запросов к API)
    if (loadReturnsCacheFromFile()) {
      returnsCacheFullyLoaded = true
      return { rows: returnsByDemandCache.get(demandId) || [] }
    }

    // Строим кеш: загружаем все возвраты постранично
    // Фильтр demand={id} НЕ поддерживается МойСклад (ошибка 1034),
    // поэтому загружаем все возвраты и фильтруем на клиенте.
    console.log('[order.js] Building returns cache from API...')
    let offset = 0
    const PAGE_SIZE = 100
    let totalLoaded = 0

    while (true) {
      const page = await API.GET('entity/salesreturn', {
        limit: PAGE_SIZE,
        offset: offset
      })
      ...
      for (const r of rows) {
        if (r.demand?.meta?.href) {
          const dId = r.demand.meta.href.split('/').pop()
          if (!returnsByDemandCache.has(dId)) {
            returnsByDemandCache.set(dId, [])
          }
          returnsByDemandCache.get(dId).push({
            id: r.id,
            name: r.name,
            sum: r.sum,
            demand: r.demand
    ...
    returnsCacheFullyLoaded = true
    console.log(`[order.js] Returns cache built: ${totalLoaded} returns, ${returnsByDemandCache.size} demands`)
...
// Функция для принудительного обновления кеша (вызывается после создания нового возврата)
function invalidateReturnsCache() {
  returnsCacheFullyLoaded = false
  // Удаляем файл кеша, чтобы при следующем вызове перестроить
  try {
    if (fs.existsSync(CACHE_FILE)) {
      fs.unlinkSync(CACHE_FILE)
    }
  } catch (e) {
    console.warn('[order.js] Failed to delete cache file:', e.message)
  }
}
...
async function findSalesReturnsByDemand_v2(demandId, demandName, shipmentNum) {
  try {
    ...
    console.error('Error finding sales return by demand v2:', e.message)
    return { rows: [] }
  }
}
