...
/**
 * @file Модуль пакетной обработки заказов
 * @description Содержит функции для пакетной обработки заказов с SSE streaming,
 * параллельной проверки и выполнения действий (отгрузка, платёж, возврат, отмена).
 */
...
const { createPayment, createPartialPayment } = require('./payment')
 ...
 * @param {Object[]} [options.checkResults] - готовые результаты проверки (передаются с фронтенда из первого сканирования).
 *   Если передан — re-check пропускается, что даёт ~8000-10000 экономии API-запросов на 2000 заказов.
...
async function processBatch(numbers, action, log = console.log, onProgress = null, options = {}) {
  const { onAbort, abortId, checkResults } = options
  const results = []
  let created = 0
  let skipped = 0
  let errors = 0
  ...
  // ── canAction: pre-checked (из первого сканирования) или повторная проверка ──
  // Если с фронтенда передан checkResults (результаты первого сканирования),
  // используем их напрямую — это экономит ~4-5 API-запросов на заказ,
  // т.е. ~8000-10000 запросов на 2000 заказов + ~2 минуты времени.
  // Каждая функция действия (createPayment, createDemand и т.д.) сама проверяет
  // корректность состояния заказа, поэтому re-check избыточен.
  let canActionResults
  if (checkResults && checkResults.length > 0) {
    canActionResults = checkResults
    log(`Using ${checkResults.length} pre-checked results, skipping re-check`)
  } else {
    canActionResults = []
    for (let i = 0; i < numbers.length; i += BATCH_CONCURRENCY) {
      if (checkAbort()) {
...
async function executeAction(checkResult, action, log) {
  try {
    ...
    case 'payment': {
      // Если есть частичный возврат — создаём частичный платёж
      const isPartialReturn = checkResult.returnSum && checkResult.returnSum > 0 && checkResult.returnSum < checkResult.sum
      const payment = isPartialReturn
        ? await createPartialPayment(checkResult.orderId)
        : await createPayment(checkResult.orderId)
      return {
        status: 'created',
        shipmentNum: checkResult.shipmentNum,
        paymentName: payment.name,
        paymentSum: payment.paymentSum || payment.sum / 100
      }
    }
    case 'return': {
