...
async function getOzonReturnsMap(clientId, apiKey, log = console.log) {
  ...
  try {
    const daysBack = ozonReturnsCache.lastDate
      ? Math.ceil((Date.now() - new Date(ozonReturnsCache.lastDate).getTime()) / (24 * 60 * 60 * 1000)) + 3
      : 120
