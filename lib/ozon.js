...
/**
 * Search both Ozon caches by number.
 * Searches ozonReturnsCache (by returnId, postingNumber, barcode, id)
 * then ozonPostingsCache (by postingNumber).
 *
...
/**
 * Refresh expired Ozon caches.
 * Calls getOzonReturnsMap and getOzonPostingsMap — each checks TTL internally
 * and only fetches if stale.
 *
...
/**
 * Force refresh all Ozon caches.
 * Resets fetchedAt to 0 so both caches re-fetch regardless of TTL.
 *
 * @param {string} ozonClientId - Ozon Client-Id
 * @param {string} ozonApiKey - Ozon Api-Key
 * @param {function} [log] - функция логирования
