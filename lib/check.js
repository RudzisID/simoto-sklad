...
async function checkOrder(shipmentNum, log = console.log) {
  ...
  // Прямой поиск возвратов, привязанных к этой отгрузке.
  // expand=returns в getDemand может не вернуть возвраты (обратная связь МойСклад не всегда работает),
  // поэтому делаем отдельный поиск через кеш всех возвратов с фильтром по demand.meta.href.
  const returnsByDemand = await findSalesReturnsByDemand(demandId)
  if (returnsByDemand?.rows?.length > 0) {
    if (!demand.returns) demand.returns = { rows: [] }
    if (!Array.isArray(demand.returns.rows)) demand.returns.rows = []
    const existingIds = new Set(demand.returns.rows.map(r => r.id))
    for (const r of returnsByDemand.rows) {
      if (!existingIds.has(r.id)) {
        demand.returns.rows.push(r)
      }
    }
  }
  ...
  // Вычисляем сумму всех возвратов (в копейках)
  ...
  if (allReturns.length > 0) {
    console.log(`[check.js] Found ${allReturns.length} returns for ${shipmentNum}:`, 
      allReturns.map(r => ({ name: r.name, sum: r.sum })))
    returnSumKopeks = allReturns.reduce((acc, r) => {
      const sum = r.sum || 0
  ...
  return {
    ...
    returnSum: returnSumKopeks / 100, // Сумма возвратов в рублях
    status: 'other',
    statusName,
    canDemand,
    canPayment,
 ...
 * WB: ищем после "Номер задания в Wildberries"
 */
