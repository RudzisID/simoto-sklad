const { findOrderByShipmentNum, getOrderFull, getOrderFullForCreate, getDemand, findSalesReturnsByDemand_v2, findSalesReturns } = require('./order');
...
async function checkOrder(shipmentNum, log = console.log) {
    ...
    if (!returnName && !hasReturn && orderFull.demands && orderFull.demands.length > 0 && (isCancelled || stateName.toLowerCase().includes('возврат'))) {
    ...
    if (!returnName && orderFull.returns && orderFull.returns.rows && orderFull.returns.rows.length > 0) {
