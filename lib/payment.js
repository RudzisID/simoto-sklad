...
/**
 * Безопасно меняет статус отгрузки после создания платежа.
 * Ошибка настройки статусов у конкретного пользователя не отменяет платёж.
 * @param {Object} API - Клиент API МойСклад
 * @param {Demand} demand - Отгрузка МойСклад
 * @param {Order} orderFull - Полные данные заказа
 * @param {string} targetStatusId - UUID целевого статуса отгрузки
 * @param {string} logPrefix - Префикс сообщения в логе
 * @returns {Promise<void>}
 */
async function updateDemandPaymentStatus(API, demand, orderFull, targetStatusId, logPrefix) {
  const currentStateId = demand.state?.meta?.href?.split('/').pop()
  if (currentStateId !== DEMAND_STATUS.ON_PAYMENT) {
    return
  }

  try {
    await API.PUT('entity/demand/' + demand.id, {
      state: {
        meta: {
          href: `https://api.moysklad.ru/api/remap/1.2/entity/demand/metadata/states/${targetStatusId}`,
          type: 'state'
        }
      },
    ...
    console.log(`${logPrefix}: не удалось изменить статус отгрузки ${demand.id}: ${error.message}`)
  }
}
...
async function createPayment(orderIdOrFull, demandIdOrObj) {
  ...
  try {
    ...
    await updateDemandPaymentStatus(API, demand, orderFull, DEMAND_STATUS.PAID, '[Payment]')
    return payment
  } catch (error) {
...
async function createPartialPayment(orderIdOrFull, demandIdOrObj) {
  ...
  const demand = await API.GET('entity/demand/' + demandId + '?expand=returns,positions,state,salesChannel')
  ...
  try {
    ...
    await updateDemandPaymentStatus(API, demand, orderFull, DEMAND_STATUS.PARTIALLY_PAID, '[Partial payment]')
